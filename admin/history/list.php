...
<body>
	...
	$status=$_GET['status'];
	if($status=='approved'){
	$qry="SELECT * FROM `history` WHERE `status`='Approved'";//fetch applicants whose remark is present
	}else{
	$qry="SELECT * FROM `history` WHERE `status`='Rejected'";//fetch applicants whose remark is absent
	}
	...
	while($row=mysqli_fetch_assoc($run))
	{
		...
		$city=$row2['name'];
		$areaid1=$row['quater_area1'];
		$areaid2=$row['quater_area2'];
		$areaid3=$row['quater_area3'];
		$areaid4=$row['quater_area4'];
		...
		?>
                ...
                <a href="remarked.php?user_id=<?php echo $id ;?>&city=<?php echo $apply_city ?>&applicant=<?php echo $applicant; ?>" class="list-group-item list-group-item-action flex-column align-items-start zoom">
                    ...
                    <p class="mb-1">Current allotment <?php echo $city; ?>| Applied for <?php echo $apply_city; ?></p>
